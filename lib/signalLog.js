...
const HEADER = 'date,symbol,signal,price,atr,ema200,rsi,combo';
...
export function logSignals(results) {
  ...
  for (const r of results) {
    const activeSignals = getActiveSignals(r);
    // Every row for this (date, symbol) carries the same combo — the full set
    // of signals that fired together that day — so a later query can group on
    // it directly instead of re-deriving co-occurrence from (date, symbol).
    const combo = [...activeSignals].sort().join('+');
    for (const signal of activeSignals) {
      const key = `${r.date},${r.symbol},${signal}`;
      if (seen.has(key)) continue;
      seen.add(key);
      rows.push(`${key},${r.price},${r.atr},${r.ema200 ?? ''},${r.rsi},${combo}`);
    }
  }
