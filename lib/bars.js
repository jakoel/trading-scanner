...
/**
 * Backfill for a session Yahoo's daily chart endpoint has left incomplete.
 *
 * Yahoo occasionally leaves a just-closed session's OHLC null in the `1d`
 * chart series for hours after the close (observed 2026-09-22 across
 * unrelated symbols — AAPL, MSFT, SPY, NVDA, TSLA all affected), which makes
 * fetchChart() silently drop that row. The live quote endpoint already has
 * the finalized print by then, so use it as a fallback for exactly that one
 * missing session — never for anything fetchChart() already returned.
 *
 * Returns null if the quote's last regular-session close isn't dated `cutoff`
 * (e.g. cutoff was a non-trading day, or the quote is stale too).
 */
async function fetchQuoteBar(symbol, cutoff) {
  const q = await yf.quote(symbol);
  if (!q.regularMarketTime) return null;
  if (easternParts(q.regularMarketTime).date !== cutoff) return null;
  const { regularMarketOpen: open, regularMarketDayHigh: high, regularMarketDayLow: low, regularMarketPrice: close } = q;
  if (open == null || high == null || low == null || close == null) return null;
  return { date: cutoff, open, high, low, close, volume: q.regularMarketVolume ?? 0 };
}
...
/**
 * The newest session date held for any symbol, or null if nothing is stored.
 *
 * Read *before* updateBars() runs, this is the session the previous run
 * reported on — which is what the staleness guard in scan_headless.js compares
 * against. See that guard for why it exists.
 */
export function latestStoredSessionDate() {
  const row = getDb().prepare('SELECT MAX(date) AS date FROM bars').get();
...
export async function updateBars(symbol) {
  ...
  if (!fresh.some(row => row.date === cutoff)) {
    const quoteBar = await fetchQuoteBar(symbol, cutoff);
    if (quoteBar) fresh = [...fresh, quoteBar].sort((a, b) => a.date.localeCompare(b.date));
  }
