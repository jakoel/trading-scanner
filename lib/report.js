...
export function formatTelegramMessages(results) {
  ...
  // No standalone MACD Turned Green section — MACD Green alone (52-61% win
  // depending on horizon) is weaker than either confluence section above, so
  // it only surfaces here via a combo, not on its own.
  ...
  if (!sections.length && !macdPositiveCount && !rsiReclaimCount && !volumeSurgeCount) return [];
