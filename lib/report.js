...
/**
 * Header naming the trading session the report covers. Entries all carry the
 * same bar date in practice; the newest is used so a symbol lagging a day
 * (a halt, a late listing) can't backdate the whole report.
 */
function formatHeader(results) {
  ...
  return `*Watchlist Scan* — session of ${label}\n`;
}
...
export function formatTelegramMessages(results) {
  ...
  const messages = [];
  // The scan is dispatched before the US open, so it reports on the previous
  // completed session — state the bar's own trading date rather than leaving
  // the reader to assume the report covers the day they received it.
  let current = formatHeader(results);
  ...
  const volumeSurge = results.filter(r => r.volumeSignals.includes('VOLUME SURGE'));

  const detailFormatters = {
    rsi: r => `*${r.symbol}* $${r.price} (RSI ${r.rsi})\n`,
    volume: r => `*${r.symbol}* $${r.price} (${r.volumeRatio.toFixed(2)}x avg vol)\n`,
  };

  for (const [title, items, detail] of [
    ['⚡ MACD Turned Positive (Confirmed Positive Momentum)', macdPositive, null],
    ['⚡ MACD Turned Green (Possible Bottom / Momentum Shift)', macdGreen, null],
    ...
    for (const r of items) {
      secText += detail ? detailFormatters[detail](r) : `*${r.symbol}* $${r.price}\n`;
    }
    if ((current + secText).length > 3800) {
      messages.push(current.trim());
      current = secText;
