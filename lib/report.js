 ...
 */
// Re-exported so scan_headless.js/scan_watchlist.js can import every detectX
// function from this one module, same as the MACD/RSI/volume detectors below.
export { detectFvgSignals, detectVwapSignals } from './fairValue.js';
...
export function formatTelegramMessages(results) {
  ...
  // Same-day (well, +/-3 trading day windowed match, anchored on the later
  // occurrence — see research/brute-force.mjs) Bullish FVG Retest + RSI
  // Reclaimed 30. The most broadly validated new find: n=109 watchlist (61%
  // win), n=205 S&P 500 (62% win), n=101 combined watchlist+QQQ (61% win) at
  // 5 sessions — including the full S&P 500, not just momentum-tilted names.
  const comboFvgRsi = sorted.filter(r =>
    r.fvgSignals.includes('BULLISH FVG RETEST') && r.rsiSignals.includes('RSI RECLAIMED 30'));
