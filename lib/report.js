...
export function formatTelegramMessages(results) {
  ...
  const bullishDivAdx = sorted.filter(r =>
    r.divergenceSignals.includes('BULLISH DIVERGENCE (Strong Trend)'));

  function formatStock(r) {
    return `*${r.symbol}* $${r.price}${sectorTag(r.symbol)}\n  _${r.summary}_`;
  }

  // No standalone MACD Turned Green section — it only surfaces via the Strong
  // Trend combo above. RSI Reclaimed 30 has no display anywhere at all as of
  // 2026-09-22 — see "Removed: MACD Green + Volume Surge and RSI Reclaim +
  // Volume Surge" below for why. detectMacdSignals()/detectRsiSignals() still
  // tag every occurrence of both for data/signals.csv research logging.
  ...
  const sections = [];
  if (bullishDivAdx.length) sections.push({ title: '🔥 Bullish Divergence + Strong Trend (Confluence)', items: bullishDivAdx });
