...
| 🔥 BULLISH FVG FORMED / RETEST | `lib/fairValue.js`'s `detectFvgSignals()` — a Fair Value Gap (3-candle price imbalance) ported from LuxAlgo's public Pine indicator, and its retest (price returning to an unfilled gap and finding support). Not part of `lib/indicators.js`. **Gap formation also requires a displacement filter** (`FVG_DISPLACEMENT_ATR_MULT`, 1.0x ATR(14) on the impulse candle) — see below, including why this is a provisional, not fully cross-validated default. Only RETEST is used in a shipped combo (via a windowed match, not same-day — see below); FORMED is logged but not displayed. |
...
**MACD Green has no standalone Telegram section.** Alone, its win rate (52-61% depending on horizon) is weaker than the confluence section it feeds into, so as of 2026-09-22 it's dropped from the per-signal loop in `formatTelegramMessages()` — it only surfaces via `🔥 MACD Green + Strong Trend`. `detectMacdSignals()` still tags every occurrence (needed for that combo, and `data/signals.csv` still logs standalone `MACD TURNED GREEN` rows for research) — this is a display-only change, not a removal of the signal like ATR Reclaim's.

**RSI Reclaimed 30 and Bullish Divergence + Strong Trend have no Telegram display at all, standalone or combo, as of 2026-09-22** — see the two "Removed" sections below. `detectRsiSignals()`/`detectDivergenceSignals()` still tag every occurrence of both for `data/signals.csv` logging.

### Removed: MACD Green + Volume Surge, and RSI Reclaim + Volume Surge confluence sections

Both were originally shipped on small-sample watchlist evidence: `MACD TURNED GREEN` + `VOLUME SURGE` on 59 same-day occurrences in `data/bars.db` (+3.85%/64% win at 10 sessions), `RSI RECLAIMED 30` + `VOLUME SURGE` at a tightened 2.0x volume ratio on 38 occurrences (+7.69%/82% win at 20 sessions, later +13.15%/85% at 30d). Both looked solid at the time and shipped.

They didn't hold up once tested against broader, independent stock universes — the same `research/` brute-force tooling used to find Bullish Divergence + Strong Trend, pointed at three more datasets (all 200 trading days each, fetched separately from `data/bars.db`): the S&P 500 (503 symbols, `research/universe.db`), Nasdaq-100/QQQ (101 symbols, `research/nasdaq100.db`), and a combined watchlist+QQQ set (174 symbols, `research/watchlist_qqq.db`):

| Combo | Watchlist (2yr) | S&P 500 (200d) | Nasdaq-100 (200d) | Combined (200d) |
|---|---|---|---|---|
| MACD Green + Volume Surge (10d) | 64% win | 46-49% win | 29-50% win | 46-61% win |
| RSI Reclaim + Volume Surge (20-30d) | 82-85% win | 38-48% win | 43-50% win | 48-57% win |

Both went weak-to-flat, sometimes worse than a coin flip (MACD Green + Volume Surge hit 29% win on Nasdaq-100 at 20d, with a negative average return) — off the watchlist they're not real. Bullish Divergence + Strong Trend replicated on all three broader datasets, which looked like the opposite pattern at first — see below for why it got removed anyway.

Before removing, a dozen candidate fixes were tried on each (ADX gates at 20/25, a higher volume-ratio cutoff, RSI-50 cross, ATR reclaim same day, above-EMA200, a third confluence leg) across all four datasets, looking for something that would behave the way `bullFading` did for MACD Positive (see the follow-up MACD Positive research below) — consistently better everywhere. None did; every candidate helped on some datasets and hurt on others, or collapsed the sample to single digits with untrustworthy swings. Concluded these are watchlist-specific artifacts (or plain small-sample overfitting), not fixable interactions, and removed both sections. `RSI_VOLUME_COMBO_MIN_RATIO` (the report-section-only volume threshold this combo used) was deleted along with it.

### Removed: Bullish Divergence + Strong Trend confluence section

This one's discovery story is the opposite of the two above, which is exactly why it took longer to catch. `indicators.js`'s pivot-confirmed bullish divergence flag (`row.divergence === 'BULLISH DIV'`) had never been wired into any signal before this session — see "Momentum and Volume cells" below for the sibling story of *why* some indicator fields go unused. Found by running `research/brute-force.mjs` against the S&P 500 (503 symbols, 200 trading days each, `research/universe.db`): paired with ADX ≥ 20, n=157 at 20 sessions (+4.05%/71% win), n=144 at 30 (+5.68%/70% win) — beating both legs alone and, unusually, getting *stronger* in the more recent half of that dataset's history rather than weaker. It then replicated on Nasdaq-100/QQQ (80%/73% win) and the combined watchlist+QQQ set (72%/64% win) — three independent broader datasets all agreeing, the strongest cross-validation of anything found this session. Shipped on that basis.

It never actually worked on the watchlist itself. Checked directly against `data/bars.db` (both the full ~2-year history and just the last 200 trading days, to match the other datasets' window): 49-53% win at every horizon, trending *down* as the horizon lengthens (53% at 5d → 49% at 30d on full history) — the opposite of its behavior everywhere else. The combined watchlist+QQQ set's 72%/64% had looked like confirmation, but the edge there was apparently coming entirely from the ~70 QQQ-only names in that 174-symbol set, not from the ~104 watchlist symbols sitting inside it. The lesson generalizes: a combo validating on a broader universe is necessary but not sufficient — it still has to be checked against the actual symbols the scanner runs on, not just "a big diverse sample that happens to contain them." `BULLISH_DIV_MIN_ADX` (20, `lib/report.js`) and `detectDivergenceSignals()` are kept (unlike `RSI_VOLUME_COMBO_MIN_RATIO`, which was deleted) since `data/signals.csv` still logs occurrences for research — removing the Telegram section didn't require removing the underlying detection.
...
### Bullish FVG Retest + RSI Reclaim was shipped broken, then fixed and re-tuned

The row above (n=109/205/101, 61-62% win at 5d) was validated using `research/brute-force.mjs`'s windowed match — flags counted as a combo if they fired within `+/-3` trading days of each other, anchored on the later occurrence. But the first ship of this section only checked whether both signals were true on the *exact same* result entry, since a live daily scan only ever computes "today's" signals. Checked directly against all four datasets: that same-day version fired **0 to 8 times total**, across hundreds of trading days each — it never reflected what was validated and essentially never displayed.

Fixed with `detectFvgRsiConfluence()` (`lib/report.js`): computed per symbol in `scan_headless.js` from the raw `bars`/`rows` history (not the single-day signal arrays), it fires on today's bar if today hosts one of the two signals and the other fired within the last `FVG_RSI_COMBO_WINDOW` (3) trading days, today included. A live scan can only ever anchor on "today" — there's no future bar to look ahead to — so this is the correct one-sided version of the backtest's two-sided window, and was checked for lookahead bias directly (evaluating a bar before its own window's RSI reclaim happens correctly returns false).

While fixing this, also revisited the underlying FVG retest logic itself (`lib/fairValue.js`):

- **Multi-zone tracking fix**: `detectFvgSignals()` previously stopped scanning backward for an open zone the moment it hit the *first* mitigated one, even if an older, still-unmitigated zone existed further back. Fixed to check every unmitigated zone in the lookback window. In practice this barely moved any number (OLD vs FIXED differ by ~1pp everywhere) — a correctness fix, not a performance lever.
- **Displacement filter, shipped as the new default** (`FVG_DISPLACEMENT_ATR_MULT` = 1.0, `lib/fairValue.js`): requires the impulse candle's body to be at least 1x its own rolling ATR(14), filtering out gaps formed by insignificant candles. Re-running the windowed backtest with this filter on: watchlist 10d win 41%→**76%** (n=227→33), 30d 53%→**79%** (n=227→33). **But it made the S&P 500 result worse, not better** (10d 62%→52%, 30d 43%→**34%**, n=249→44) — the identical "strong on the watchlist, weak on the broader market" pattern that already got Bullish Divergence removed once in this same file. A volume filter (1.75x, matching `VOLUME_SURGE_THRESHOLD`) was also tested and was just noisy either way (42-73% depending on dataset/horizon, n=14-28) — not shipped.

The displacement filter was shipped anyway, on explicit instruction, not because the cross-dataset evidence supports it — this file's own established bar (cross-dataset agreement) says it shouldn't have been. Flagging this plainly so it isn't mistaken for a validated decision later: **if this section's live performance disappoints, the displacement filter is the first thing to suspect and the easiest to revert** — pass `{ dispMult: 0 }` to `bullishFvg()` (or change `FVG_DISPLACEMENT_ATR_MULT` to `0`) to go back to the un-filtered, more broadly-validated version.
...
A structural caveat specific to the 200-day universe datasets (S&P 500, Nasdaq-100, the combined set): capped at 200 trading days per symbol, EMA200 never gets enough history to produce a single value (it needs 200 values just for its first output), so `aboveEma200`, `trendBullish`, and `confirmedBuy` were all unusable there (n=0) — any finding involving those flags on those datasets would be meaningless, and none were used. Separately, most other single-signal baselines were also visibly weaker on the S&P 500 (mostly 50-54% win) than on the watchlist (55-63%) — the watchlist is hand-picked toward higher-beta growth/momentum names, where trend-following signals plausibly work better than on the broader index. Treat every signal in this file as tuned to *this* watchlist, not a demonstrated universal market edge.
