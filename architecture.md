...
**MACD Green and RSI Reclaim have no standalone Telegram section.** Alone, MACD Green's win rate (52-61% depending on horizon) and RSI Reclaim's (52-62%, barely better than a coin flip at 10d even with the ADX gate on) are both weaker than the confluence sections they feed into, so as of 2026-09-22 both are dropped from the per-signal loop in `formatTelegramMessages()` — MACD Green only surfaces via `🔥 MACD Green + Volume Surge` or `🔥 MACD Green + Strong Trend`, RSI Reclaim only via `🔥 RSI Reclaim + Volume Surge`. The underlying `detectMacdSignals()`/`detectRsiSignals()` still tag every occurrence (needed for the combos, and `data/signals.csv` still logs standalone `MACD TURNED GREEN` and `RSI RECLAIMED 30` rows for research) — this is a display-only change for both, not a removal of the signal like ATR Reclaim's.
