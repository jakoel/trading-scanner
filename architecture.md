...
**Testing/dry runs:** `node scan_headless.js --dry-run` prints the would-be report to stdout instead of calling `sendMessage()`. Bar data (`data/bars/*.csv`) and the signal log (`data/signals.csv`) still update normally — only the Telegram send is skipped. Always use this when verifying new or changed signal logic; the live channel has real subscribers and a stray test post is noisy for them.
