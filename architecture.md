...
**RSI Reclaimed 30 has no Telegram display at all, standalone or combo, as of 2026-09-22** — see "Removed: MACD Green + Volume Surge and RSI Reclaim + Volume Surge" below. `detectRsiSignals()` still tags every occurrence for `data/signals.csv` logging.
...
Both went weak-to-flat, sometimes worse than a coin flip (MACD Green + Volume Surge hit 29% win on Nasdaq-100 at 20d, with a negative average return) — off the watchlist they're not real. Unlike Bullish Divergence + Strong Trend, which replicated on all three broader datasets, these two only ever worked on the small sample they were originally found on.
...
**Bullish Divergence + Strong Trend confluence section.** All prior combo research in this file came from the ~104-symbol watchlist. This one came from running the same brute-force search (`research/brute-force.mjs`) against a much larger, independent cross-section: all 503 S&P 500 symbols, 200 trading days each, fetched 2026-09-22 via `research/fetch-universe.mjs` into `research/universe.db` — entirely separate from `data/bars.db`, which was never touched. `indicators.js`'s pivot-confirmed bullish divergence flag (`row.divergence === 'BULLISH DIV'`) had never been wired into any signal before this — see "Momentum and Volume cells" below for the sibling story of *why* some indicator fields go unused. Paired with ADX ≥ 20: n=157 at 20 sessions (+4.05%/71% win), n=144 at 30 (+5.68%/70% win) — beating both legs alone (bullish divergence alone: 61% win; ADX ≥ 20 alone: 51%, both on the S&P 500 dataset) and, unlike almost every combo found this session, getting *stronger* in the more recent half of history (67% → 71% at 20d) rather than weaker. `BULLISH_DIV_MIN_ADX` (20) gates it — same value as `RSI_RECLAIM_MIN_ADX` by coincidence of two independent backtests, not reuse; it has its own constant. `detectDivergenceSignals()` only ever produces this one combo signal, never a standalone "BULLISH DIVERGENCE" — that wasn't separately validated, so it isn't shipped. `adx` is optional; the legacy CDP scanner doesn't scrape it, so this can never fire there.

A structural caveat specific to the S&P 500 dataset this was found on: capped at 200 trading days per symbol, EMA200 never gets enough history to produce a single value (it needs 200 values just for its first output), so `aboveEma200`, `trendBullish`, and `confirmedBuy` were all unusable there (n=0) — any finding involving those flags on that dataset would be meaningless, and none were used. Separately, most other single-signal baselines were also visibly weaker on the S&P 500 (mostly 50-54% win) than on the watchlist (55-63%) — the watchlist is hand-picked toward higher-beta growth/momentum names, where trend-following signals plausibly work better than on the broader index. Treat every signal in this file as tuned to *this* watchlist, not a demonstrated universal market edge.
...
```
*🔥 Bullish Divergence + Strong Trend (Confluence):*
*SYMBOL* $price
  _above EMA200_
